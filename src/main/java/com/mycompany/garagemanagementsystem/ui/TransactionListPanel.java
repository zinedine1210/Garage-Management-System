...
public class TransactionListPanel extends javax.swing.JPanel {
    ...
    private void applyFilter() {
        ...
        if (text.length() > 0) filters.add(javax.swing.RowFilter.regexFilter("(?i)" + text));
        String status = cbStatus.getSelectedItem().toString();
        if (!status.equals("Semua")) filters.add(javax.swing.RowFilter.regexFilter("(?i)^" + status + "$", 8));
        String bulan = cbBulan.getSelectedItem().toString();
        if (!bulan.equals("Semua")) filters.add(javax.swing.RowFilter.regexFilter("-[0]*" + bulan + "-", 1));
